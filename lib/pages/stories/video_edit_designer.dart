...
/// Video editor page using VSStoryDesigner package
class VideoEditorDesignerPage extends StatefulWidget {
  final String path;
  const VideoEditorDesignerPage({super.key, required this.path});

  @override
  State<VideoEditorDesignerPage> createState() =>
      _VideoEditorDesignerPageState();
}

class _VideoEditorDesignerPageState extends State<VideoEditorDesignerPage> {
  bool _isValid = true;
  ...
  Future<void> _validateFile() async {
    try {
      ...
      if (!ready) {
        setState(() {
          _isValid = false;
          _errorMessage = 'Video file is not ready. Please try again.';
        });
        return;
      }
  ...
  Widget build(BuildContext context) {
    ...
    // VSStoryDesigner expects a file path, not a URI
    final file = File(widget.path);
    ...
    return VSStoryDesigner(
      ...
      onDone: (uri) {
        ...
        if (mounted) {
          Routers.goToAddStoryPage(context, videoPath: uri);
        }
      },
      mediaPath: mediaPath,
    );
