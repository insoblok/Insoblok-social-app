    ...
    return ViewModelBuilder<WalletSendProvider>.reactive(
      ...
      onViewModelReady:
          (viewModel) => viewModel.init(
            context,
            args["sender"].toString(),
            args["receiver"].toString(),
            args["network"].toString(),
            double.parse(args["amount"].toString()),
          ),
      builder: (context, viewModel, _) {
        final network = kWalletTokenList.firstWhere(
          (tk) => tk["chain"].toString() == viewModel.network,
        );
        final price =
            viewModel.amount! * viewModel.allPrices[viewModel.network!]!;
        final fee =
            viewModel.transactionFee *
            (viewModel.allPrices[viewModel.network!] ?? 0).toDouble();
              ...
              children: [
                ...
                Padding(
                  ...
                  child: GradientPillButton(
                    ...
                    loadingText: "Sending ...",
                    onPressed: () async {
                      await viewModel.handleClickSend(
                        context,
                        viewModel.sender ?? "",
                        viewModel.receiver ?? "",
                        viewModel.network ?? "",
