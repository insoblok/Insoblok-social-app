...
class LocalAuthService {
  ...
  Future<bool> isFaceIDAvailable() async {
    try {
      ...
      apiService.logRequest({
        "result": "failed",
        "message":
            'Error checking biometric availability: $e  : isFaceIDAvailable',
      });
      return false;
    }
  }
  ...
  Future<bool> authenticateWithBiometrics({
    ...
    if (!available) {
      ...
      apiService.logRequest({
        "result": "failed",
        "message":
            'Face Unlock is not available: authenticateWithBiometrics ${await AIHelpers.getIPAddress()}',
      });
    ...
    try {
        ...
        authMessages: [
          AndroidAuthMessages(
            ...
            deviceCredentialsSetupDescription:
                'Device credential setup description',
      ...
      apiService.logRequest({
        "result": "failed",
        "message":
            'localAuth.authenticate result is $isAuthenticated : authenticateWithBiometrics ${await AIHelpers.getIPAddress()}',
      });
      ...
      apiService.logRequest({
        "result": "Failed",
        "message":
            "Biometric authentication error: $e  : authenticateWithBiometrics",
      });
      return false;
    }
  }
  ...
  Future<UnlockedWallet> accessWalletWithFaceId() async {
    UnlockedWallet result = UnlockedWallet(
      address: "",
      mnemonic: "",
      privateKeyHex: "",
    );
    try {
      ...
      if (!authenticated) {
        apiService.logRequest({
          "result": "Failed",
          "message": "authresult is false: accessWalletWithFaceId",
        });
        return result;
      }
      ...
      apiService.logRequest({
        "result": "Failed",
        "message":
            "Error accessing wallet: ${e.toString()}  : accessWalletWithFaceId",
      });
    }
    return result;
  }
