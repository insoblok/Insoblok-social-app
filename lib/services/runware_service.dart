...
class RunwareService {
  ...
  //seed edit 3.0
  Future<Map<String, dynamic>> generateAIAvatarWithPromptOption1({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Avatar generation response: $responseData');

        // Parse immediate response - imageURL is already in the response
        // Response structure: { "data": [{ "taskUUID": "...", "imageURL": "...", "cost": ... }] }
        if (responseData is Map<String, dynamic>) {
          if (responseData.containsKey('data') &&
              responseData['data'] is List) {
            final data = responseData['data'] as List;
            if (data.isNotEmpty && data[0] is Map) {
              final taskData = data[0] as Map;
              final imageURL = taskData['imageURL'] as String?;

              if (imageURL != null && imageURL.isNotEmpty) {
                return {
                  'status': 'success',
                  'success': true,
                  'taskUUID': taskData['taskUUID'] ?? taskUUID,
                  'imageURL': imageURL,
                  'cost': taskData['cost'],
        ...
        // If response structure is unexpected, throw an error
